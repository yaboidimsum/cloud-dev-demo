...
export default function FeedbackPopOver() {
  ...
  return (
    ...
    >
      ...
      >
        <motion.span layoutId="title" className="font-semibold text-zinc-800">
          Message
        </motion.span>
      </motion.button>
      <AnimatePresence>
        {open ? (
          <motion.div
            ref={ref}
            layoutId="wrapper"
            className={`${FeedbackPopOverStyle.feedbackPopover}`}
            style={{ borderRadius: 12 }}
          >
            <motion.span
              aria-hidden
              className={`${FeedbackPopOverStyle.placeholder} `}
              layoutId="title"
            ...
            <AnimatePresence mode="popLayout">
              {formState === "success" ? (
                ...
                >
                  ...
                  <h3>Message sent!</h3>
                  <p>Our troubles are all the same</p>
                ...
                <motion.form
                  ...
                  className={`${FeedbackPopOverStyle.feedbackForm}`}
                >
                  <textarea
                    autoFocus
                    placeholder="Feedback"
                    onChange={(e) => setFeedback(e.target.value)}
                    className={`${FeedbackPopOverStyle.textarea} text-zinc-800`}
                    required
                  />
                  <div className={`${FeedbackPopOverStyle.feedbackFooter}`}>
                    <svg
                      className={`${FeedbackPopOverStyle.dottedLine}`}
                      width="352"
                      height="2"
                      viewBox="0 0 352 2"
                      fill="none"
                    ...
                    <div className={`${FeedbackPopOverStyle.halfCircleLeft}`}>
                      <svg
                        width="6"
                        height="12"
                        viewBox="0 0 6 12"
                    ...
                    <div className={`${FeedbackPopOverStyle.halfCircleRight}`}>
                      <svg
                        width="6"
                        height="12"
                        viewBox="0 0 6 12"
                    ...
                    <div className="flex w-full justify-between">
                      <button
                        type="button"
                        className={`${FeedbackPopOverStyle.closeButton} ease w-fit transform cursor-pointer px-2 duration-150 active:scale-95`}
                      ...
                      <button
                        type="submit"
                        className={`${FeedbackPopOverStyle.submitButton} ease w-fit transform cursor-pointer px-2 duration-150 active:scale-95`}
                      >
